...
void part1() {
    ...
    std::string line;       
    std::string word;   
    std::ifstream in_file {"/workspaces/docker_cpp/volansys_cpp_advanced/16_File_IO/files/words.txt"};
    if (in_file) {

        // You implement this code
        while (in_file)
        {
            std::getline(in_file, line);  //Get the line

            std::stringstream ss(line);  //Make a stream out of line

            while (ss >> word)  //Take the word
                words[word]++;  //Now if the word repeats, its frequency is increased
        }
...
void part2() {
    ...
    if (in_file) {
        ...
        while(in_file)
        {
            ...
            while (ss >> word)
            {
                words[word].insert(line_no);   //For that map, insert line no (which is unique) into its set
            }
