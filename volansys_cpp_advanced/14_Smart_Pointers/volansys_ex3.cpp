 ...
 * @author rohit S
 * @brief Challenge 5 of C++ Advance
 * @version 0.1
 * @date 2023-12-19
 * 
...
std::unique_ptr<std::vector<std::shared_ptr<Test>>> make()
{
    // std::vector<std::shared_ptr<Test>> shared_vec;

    // std::shared_ptr<Test> p(new Test());

    // shared_vec.push_back(p);

    //auto a = std::make_unique<std::vector<std::shared_ptr<Test>>>(new Test());
...
void fill(std::vector<std::shared_ptr<Test>> &vec, int num)
{
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        std::cin >> temp;
        auto p = std::make_shared<Test>(temp);
        vec.push_back(p);
    }
